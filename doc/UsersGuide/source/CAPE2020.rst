...
.. note:: 
   ...
   In order to generate all necessary configuration, data, input, and fix files to run in this method, the user first needs to run the base RT script for the July 2020 CAPE case as described in :numref:`Section %s <run-2020-cape>`. 
...
For example, users will need to copy the ``gfs*.nc`` and ``sfc*.nc`` files from ``OUTDIR/gfs.YYYYMMDD/HH/model_data/atmos/input`` into the ``INPUT`` directory of a UFS WM run directory. (The run directory is set in ``create_xml.py`` as ``${PTMP}/${USER}/FV3_RT/rt_${pid}`` for HSD cases.) Note that this will only work when the run directory uses ICs of the same resolution. For example, if using ICs with a C192 resolution, users will need to put them in a run directory created by :ref:`running an RT <run-wm>` such as :wm-repo:`control_c192 <blob/develop/tests/tests/control_c192>`.

Additionally, users will need to adjust the model start time in the ``model_configure`` file if using a different date for ICs. For example: 
