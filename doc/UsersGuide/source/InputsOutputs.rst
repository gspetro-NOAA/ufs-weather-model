...
This chapter describes the input and output files needed for executing the model in the various supported configurations listed in :numref:`Table %s <UFS-configurations>`.
...
However, ``nems.configure`` files for other configurations of the Weather Model are more complex. A number of samples are available below:
   ...
   * :doc:`ATMAQ <samples/nems.configure.ATMAQ>`
   ...
   * :doc:`S2SW <samples/nems.configure.S2SW>`
   * :doc:`S2SWA <samples/nems.configure.S2SWA>`
   * :doc:`NG-GODAS <samples/nems.configure.NG-GODAS>`
   * :doc:`HAFS <samples/nems.configure.HAFS>`
