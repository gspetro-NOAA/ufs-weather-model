...
def initialize_PlotManager(test_data, category):
   """Check that PlotManager is initialized properly."""
   plot_manager = PlotManager(test_data,category)
   assert plot_manager.data == test_data
   assert plot_manager.category == category

@pytest.mark.parametrize('category', ['runtime', 'memory'])
def test_get_test_names(test_data, category):
...
def test_detect_statistical_anomalies(test_data):
   ...
   plot_manager = PlotManager(test_data,'memory')
...
def test_rearrange_hashes(test_data, hashes):
   plot_manager = PlotManager(test_data,'runtime')
   plot_manager.hashes = plot_manager.rearrange_hashes()
   hashes = ['PR Head'] + get_hashes()
   hashes.reverse()
   assert plot_manager.hashes == hashes

@pytest.mark.parametrize('category', ['runtime', 'memory'])
def test_generate_figure(test_data, category, hashes):
   plot_manager = PlotManager(test_data,category)
...
def test_add_test_metrics_by_machine(test_data_subset, data_by_test, category, hashes, test):
   """Test hash retrieval and metrics restructuring prior to plotting."""
   plot_manager = PlotManager(test_data_subset, category)
...
@pytest.mark.parametrize('category', ['runtime', 'memory'])
def test_plot_results(test_data_subset, category):
   """
