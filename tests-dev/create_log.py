...
def process_compile_job(apps, val, MACHINE_ID):
    COMPILE_ID = apps
    RT_COMPILER = val['compiler']
    COMPILE_LOG = f'compile_{COMPILE_ID}.log'
    COMPILE_LOG_TIME = f'compile_{COMPILE_ID}_timestamp.txt'
    ...
    try:
        with open(f'./logs/log_{MACHINE_ID}/{COMPILE_LOG}') as f:
            contents = f.read()
            if COMPILE_CHECK1 in contents or COMPILE_CHECK2 in contents:
                ...
                with open(f'./logs/log_{MACHINE_ID}/{COMPILE_LOG_TIME}') as flog:
                    timing_data = flog.read().split('\n', 1)[0]
                    etime = int(timing_data.split(",")[4]) - int(timing_data.split(",")[1])
                    btime = int(timing_data.split(",")[3]) - int(timing_data.split(",")[2])
                ...
                compile_log = f"PASS -- COMPILE {COMPILE_ID}{time_log}{warning_log}\n"
            else:
                compile_log = f"FAIL -- COMPILE {COMPILE_ID}\n"
    except FileNotFoundError:
        print(f'./logs/log_{MACHINE_ID}/{COMPILE_LOG}: does not exist')
        compile_log = f"FAIL -- COMPILE {COMPILE_ID}\n"
        ...
        for apps, jobs in rt_yaml.items():
            for key, val in jobs.items():
                if (str(key) == 'build'):
                    ...
                    if machine_check:
                        COMPILE_NR += 1
                        compile_log = process_compile_job(apps, val, env.machine_id)
                        run_logs += compile_log
                        if not "FAIL" in compile_log:
                            COMPILE_PASS += 1
